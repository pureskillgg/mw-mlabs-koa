import { createLogger, final } from '@pureskillgg/mlabs-logger'
import {
  defaultTo,
  defaultWhen,
  has,
...
export const logAndExit = ({
  ...
  try {
    const isOldNodeVer = Number(process.versions.node.split('.')[0]) < 14
    if (isOldNodeVer) return final(log, handler)
...
export default ({ config, logFilters = {} }) => {
  ...
  const getDevLogConfig = () => {
    ...
    return {
      outputFilter,
      outputMode: defaultWhenNilOrEmpty(
        outputMode,
        config.get('LOG_OUTPUT_MODE')
      )
    }
  }
