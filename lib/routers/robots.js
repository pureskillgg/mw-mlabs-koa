...
export default ({ rule = 'disallow', rules = defaultRules } = {}) => {
  if (isNotObj(rules)) {
    throw new Error(
      `${errPrefix} option 'rules' must be object, got ${typeof rules}.`
    )
  }

  if (!isNonEmptyString(rule)) {
    throw new Error(
      `${errPrefix} option 'rule' must be string, got ${typeof rule}.`
    )
  }
