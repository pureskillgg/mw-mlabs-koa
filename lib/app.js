...
import { koaHealth, koaMetrics, koaRobots, koaStatus } from './routers/index.js'
...
export default ({
  ...
  if (useLogger(logger)) {
    ...
    useDevLogger ? app.use(koaLogger(logger)) : app.use(koaProdLogger(logger))
  }
  ...
  if (useDependencyInjection(dependencyInjection)) {
    app.use(
      koaDependencyInjection({ ...dependencyInjection, requestIdParamName })
    )
  }
